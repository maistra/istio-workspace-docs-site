...
function includeShellCommand({ file }) {
    return function() {
        ...
        self.process((doc, reader, target, attrs) => {
          console.log(Object.keys(file.src.origin))
          console.log(file.src.origin)
          console.log("-------------------")
...
function doCWD(file) {
  ...
  if (typeof absolutePath !== 'undefined') {
      ...
      cwd = cwd.replace("$PWD", sourceDir); // TODO: revice?
    ...
    const org = file.src.origin
    var cmd = 'git clone ' + org.url + ' --depth 0 --branch ' + org.branch + ' --single-branch '
    repoHash = ""//Opal.hash(['cmd', cmd])
    cmd = cmd + '/tmp/' + repoHash
    console.log(cmd)
