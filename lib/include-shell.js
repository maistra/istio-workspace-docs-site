...
function resolveCWD(file, cwd) {
  //   cwd = cwd.replace("$PWD", sourceDir); // TODO: revise?
  var resolved = cwd;

  let absolutePath = file.src.abspath;
  if (typeof absolutePath !== 'undefined') { // check it differently
      ...
      if (cwd.includes("$PROJECT_DIR")) {
          ...
          if (gitRoot.status == 0) {
              resolved = cwd.replace("$PROJECT_DIR", gitRoot.stdout.toString().trimEnd());
          } else {
              process.stderr.write(gitRoot.stderr.toString());
          }
      }
  } else if (cwd.includes("$PROJECT_DIR")) {
    const origin = file.src.origin;
    const component = file.src.component;
    var cmd = 'git clone ' + origin.url + ' --depth 1 --branch ' + (origin.branch || origin.tag) + ' --single-branch '
    const repoHash = md5(cmd);
    const repoFolder = `${path.sep}tmp${path.sep}${component}-${repoHash}`;
    cmd = `${cmd} ${repoFolder}`;
    ...
    resolved = cwd.replace("$PROJECT_DIR",  repoFolder);
  }
  return resolved
}
